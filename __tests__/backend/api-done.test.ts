...
async function callDone(current: Record<string, unknown>) {
  collection.findOne.mockResolvedValue(current);
  const res = fakeRes();
...
describe("POST /api/tasks/:id/done", () => {
  ...
  it("marks a PENDING task as DONE", async () => {
    ...
    collection.findOneAndUpdate.mockResolvedValue(done);
  ...
  it("loses the atomic claim concurrently but stays idempotent (raced caller sees DONE)", async () => {
    ...
    collection.findOne.mockResolvedValueOnce(taskDoc());
    collection.findOneAndUpdate.mockResolvedValue(null);
    collection.findOne.mockResolvedValueOnce(taskDoc({ status: "DONE", doneAt: new Date() }));
  ...
  it("rejects non-POST methods", async () => {
    collection.findOne.mockResolvedValue(taskDoc());
    const res = fakeRes();
